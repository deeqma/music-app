...
function getVolIcon(volume: number, vol0Raw: string, vol2Raw: string, vol3Raw: string): string {
  if (volume === 0) return vol0Raw
  if (volume > 0.5) return vol3Raw
  return vol2Raw
}
...
export default function AudioContainer() {
  ...
  const volIcon      = getVolIcon(volume, vol0Raw, vol2Raw, vol3Raw)
